        ...
        <div>
          ...
          {activeTab === 'leaderboard' && isAdminLoggedIn && (
            <div className="w-full rounded-3xl shadow-2xl relative overflow-hidden border border-emerald-200/80 bg-gradient-to-b from-sky-300 via-sky-100 to-emerald-100 p-6">
              ...
              <div className="absolute top-8 right-12 text-white/70 text-5xl select-none">☁️</div>
              ...
              <div className="relative z-20 flex justify-between items-center mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-lg font-black text-slate-800 tracking-tight drop-shadow-sm">투표 축제 동산</h3>
                    <p className="text-[11px] text-slate-600 font-medium">1등 캐릭터를 중심으로 친구들이 강강술래를 하고 있어요!</p>
                  </div>
                </div>
                  ...
                  {ideas.length > 0 && (
                    <>
                      ...
                      {leaderIdea && (() => {
                        ...
                        return (
                          <div className="absolute z-30 flex flex-col items-center justify-center top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                            {/* 득표 정보 뱃지 */}
                            <motion.div
                              animate={{ y: [0, -4, 0] }}
                              transition={{ repeat: Infinity, duration: 1.5 }}
                              className="mb-2 px-3 py-1 bg-amber-400/95 text-slate-900 border-2 border-amber-200 rounded-full text-xs font-black shadow-lg flex items-center gap-1.5 whitespace-nowrap backdrop-blur-sm"
                            >
                              <span>👑 1위</span>
                              <span className="text-amber-950">{leaderIdea.votes_count}표</span>
                              <span className="text-[10px] opacity-80">({Math.round(leaderRatio * 100)}%)</span>
                            </motion.div>
                            ...
                            <motion.div
                              ...
                              className="w-32 h-32 flex items-center justify-center filter drop-shadow-xl cursor-pointer"
                            >
                              <LeaderSVG className="w-full h-full" />
                            </motion.div>

                            {/* 이름 및 작가 */}
                            <div className="mt-2 bg-white/90 backdrop-blur-md px-3 py-1 rounded-xl shadow-md border border-emerald-100 text-center max-w-[130px]">
                              <p className="text-xs font-black text-slate-800 truncate">{leaderIdea.title}</p>
                              <p className="text-[10px] text-slate-500 truncate">{leaderIdea.author}</p>
                            </div>
                          </div>
                        );
                      })()}

                      {/* 2등 이하 캐릭터들 (강강술래 회전 오비탈) */}
                      {otherIdeas.map((idea, idx) => {
                        ...
                        return (
                          ...
                          >
                            {/* 회전 시 캐릭터 자이로 역회전 (캐릭터가 뒤집히지 않고 정면 유지) */}
                            <motion.div
                            ...
                            >
                              {/* 득표 정보 뱃지 */}
                              <div className="mb-1 px-2.5 py-0.5 bg-white/90 text-slate-800 border border-emerald-200 rounded-full text-[11px] font-bold shadow-md flex items-center gap-1 whitespace-nowrap">
                                <span className="text-slate-500 font-extrabold">{rank}위</span>
                                <span className="text-blue-600 font-black">{idea.votes_count}표</span>
                                <span className="text-[9px] text-slate-400">({Math.round(voteRatio * 100)}%)</span>
                              </div>
                              ...
                              <motion.div
                                ...
                                className="w-20 h-20 filter drop-shadow-md"
                              >
                                <SVGComponent className="w-full h-full" />
                              </motion.div>

                              {/* 아이디어 이름 */}
                              <div className="bg-white/80 backdrop-blur-sm px-2 py-0.5 rounded-lg shadow-sm border border-emerald-100/80 text-center max-w-[100px]">
                                <p className="text-[11px] font-bold text-slate-800 truncate">{idea.title}</p>
                              </div>
                            </motion.div>
                          </motion.div>
                        );
