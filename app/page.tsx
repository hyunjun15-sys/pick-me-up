...
export default function Home() {
  ...
  // 커스텀 알림/에러 모달 상태 (브라우저 alert 팝업 대체)
  ...
  // 1. 초기 주제 목록 로드 & 주제 Realtime 구독 (새 투표 생성/삭제 시 실시간 반영)
  useEffect(() => {
    setIsMounted(true);
    fetchTopics();
  ...
  // 2. 선택된 주제의 아이디어 로드 & 아이디어 Realtime 구독 (투표 클릭 시 실시간 반영)
  useEffect(() => {
  ...
  const handleAddTopic = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase
      .from('topics')
      .insert([{ title: newTopicTitle, description: newTopicDesc }])
      .select();
  ...
  const handleDeleteTopic = async (e: React.FormEvent) => {
    ...
    setTopics(topics.filter((t) => t.id !== deleteTargetId));
  ...
  const handleSubmitIdea = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedTopic || !newTitle || !newDesc || !newAuthor) {
      setAlertMessage('모든 항목을 입력해 주세요.');
      return;
    }

    await supabase.from('ideas').insert([
      { topic_id: selectedTopic.id, title: newTitle, description: newDesc, author: newAuthor, votes_count: 0 }
    ]);
  ...
  const top5Ideas = [...ideas].sort((a, b) => b.votes_count - a.votes_count).slice(0, 5);
      ...
      {!selectedTopic ? (
        ...
        <div>
          ...
          <div className="flex gap-6 mb-6 border-b border-slate-200/80 pb-1 px-1">
            ...
            >
              <Trophy size={15} className="text-amber-500" /> 차트 현황
            </button>
          </div>
              ...
              {top5Ideas.map((idea, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm relative overflow-hidden"
                  >
                    <div className="flex justify-between items-center mb-2 z-10 relative text-xs">
                      <span className="font-bold flex items-center gap-2 text-slate-900">
                        <span className="text-blue-600 font-black">#{index + 1}</span> {idea.title}
                      </span>
                      <span className="font-extrabold text-blue-600">{idea.votes_count} 표</span>
                    </div>
      ...
      {isAdminAuthModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white p-6 rounded-2xl w-full max-w-sm border border-slate-200 shadow-2xl">
            ...
            <form onSubmit={handleAdminAuth} autoComplete="off" className="space-y-3 text-xs">
              <div>
                <input
                  type="password"
      ...
      {isTopicModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white p-6 rounded-2xl w-full max-w-sm border border-slate-200 shadow-2xl">
            ...
            <form onSubmit={handleAddTopic} autoComplete="off" className="space-y-3 text-xs">
              <input
                type="text"
      ...
      {isIdeaModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white p-6 rounded-2xl w-full max-w-sm border border-slate-200 shadow-2xl">
            ...
            <form onSubmit={handleSubmitIdea} autoComplete="off" className="space-y-3 text-xs">
              <input
                type="text"
                name="new_idea_title"
                autoComplete="off"
