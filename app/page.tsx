...
function OrbitingCharacter({
  ...
  // 원근감 스케일 및 z-index 계산 (sin 값 기준)
  // sin > 0 이면 화면 앞쪽, sin < 0 이면 화면 뒤쪽(1등 뒤)
  const depthFactor = (Math.sin(rad) + 1) / 2; // 0 (맨뒤) ~ 1 (맨앞)
  const scale = 0.75 + depthFactor * 0.4;     // 0.75 ~ 1.15
  const zIndex = Math.floor(10 + depthFactor * 30); // 10 ~ 40 (1등은 zIndex 25)
  const opacity = 0.7 + depthFactor * 0.3;     // 뒤로가면 살짝 옅어짐

  // 시선/몸 방향 (cos 값에 따라 좌/우 회전)
  const cosVal = Math.cos(rad);
  const bodyRotateY = cosVal > 0 ? -18 : 18; 

  return (
    <div
      className="absolute top-1/2 left-1/2 flex flex-col items-center pointer-events-auto transition-transform duration-75"
      style={{
            ...
            <div className="w-full rounded-3xl shadow-2xl relative overflow-hidden border border-emerald-200/80 bg-gradient-to-b from-sky-300 via-sky-100 to-emerald-100 p-6">
              ...
              >
                <AnimatePresence>
                  {ideas.length > 0 && (
                    <>
                      {/* 1등 캐릭터 (중앙 고정 / zIndex 25로 앞/뒤 캐릭터와 정확한 레이어 구분) */}
                      {leaderIdea && (() => {
