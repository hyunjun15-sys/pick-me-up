...
function OrbitingCharacter({
  ...
  const rx = 220; // 가로 궤도 크기
  const ry = 45;  // 세로 궤도 크기 (언덕 곡률에 맞춰 축소하여 공중 부양 제거)

  const baseAngle = (index / Math.max(total, 1)) * 360;
  const currentAngleDeg = (baseAngle + time * 25) % 360;
  ...
  const scale = 0.75 + depthFactor * 0.35;     
  const zIndex = Math.floor(10 + depthFactor * 30);
  const opacity = 0.8 + depthFactor * 0.2;     

  const cosVal = Math.cos(rad);
  const bodyRotateY = cosVal > 0 ? -18 : 18; 

  return (
    <div
      className="absolute top-[68%] left-1/2 flex flex-col items-center pointer-events-auto transition-transform duration-75"
      style={{
    ...
    >
      {/* 말풍선 */}
      <div className="mb-1 px-2.5 py-0.5 bg-white/90 backdrop-blur-sm text-slate-800 border border-emerald-200 rounded-full text-[10px] font-bold shadow-md flex items-center gap-1 whitespace-nowrap">
      ...
      {/* 이름표 */}
      <div className="mt-0.5 bg-white/85 backdrop-blur-sm px-2 py-0.5 rounded-lg shadow-sm border border-emerald-100/80 text-center max-w-[90px]">
                  ...
                  {ideas.length > 0 && (
                    <>
                      ...
                      {leaderIdea && (() => {
                        ...
                        return (
                          <div
                            className="absolute top-[60%] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center"
                            style={{ zIndex: 25 }}
                          >
                            <motion.div
                              animate={{ y: [0, -4, 0] }}
                            ...
                            <motion.div
                              ...
                              className="w-24 h-24 flex items-center justify-center filter drop-shadow-2xl"
                            >
