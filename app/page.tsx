...
import { Trophy, Plus, Sparkles, ThumbsUp, Check, Undo2, ArrowLeft, Trash2, FolderPlus, Lock, Unlock, Search, AlertCircle } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50/60 text-slate-800 p-6 max-w-5xl mx-auto notranslate">
      ...
      {!selectedTopic ? (
        ...
        <div>
          ...
          {/* 투표 결과 보기 (시상대 & 리더보드 연출 적용) */}
          {activeTab === 'leaderboard' && isAdminLoggedIn && (
            <div className="space-y-8 max-w-3xl mx-auto py-2">
              
              {/* 1. 상단 시상대 (Top 3 Podium) */}
              {ideas.length > 0 ? (
                <div className="flex justify-center items-end gap-3 sm:gap-6 pt-6 pb-6 border-b border-slate-100">
                  ...
                  {ideas[1] && (
                    ...
                    >
                      <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm w-full text-center mb-2 relative">
                        <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-slate-200 text-slate-700 text-[10px] font-black px-2 py-0.5 rounded-full shadow-sm border border-slate-300">
                          2위
                        </span>
                        <h4 className="font-bold text-xs text-slate-800 mt-1 line-clamp-1">{ideas[1].title}</h4>
                        <p className="text-[10px] text-slate-400 mb-1">By {ideas[1].author}</p>
                        <span className="text-xs font-black text-blue-600">{ideas[1].votes_count}표</span>
                      </div>
                      {/* 단상 기둥 */}
                      <div className="w-full h-20 bg-gradient-to-t from-slate-200 to-slate-100 rounded-t-xl border-t-2 border-slate-300 flex items-center justify-center font-black text-slate-400 text-xl shadow-inner">
                        2
                      </div>
                    </motion.div>
                  )}
                  ...
                  {ideas[0] && (
                    <motion.div
                      layout
                      transition={{ type: 'spring', stiffness: 300, damping: 25 }}
                      className="flex flex-col items-center flex-1 max-w-[180px] z-10"
                    >
                      <div className="bg-gradient-to-b from-amber-50 to-white p-4 rounded-2xl border-2 border-amber-300 shadow-xl w-full text-center mb-2 relative ring-4 ring-amber-400/20">
                        <span className="absolute -top-4 left-1/2 -translate-x-1/2 bg-amber-400 text-white text-xs font-black px-2.5 py-0.5 rounded-full shadow-md flex items-center gap-1">
                          🏆 1위
                        </span>
                        <h4 className="font-extrabold text-sm text-slate-900 mt-1 line-clamp-1">{ideas[0].title}</h4>
                        <p className="text-[10px] text-slate-500 mb-2">By {ideas[0].author}</p>
                        <span className="text-xs font-black text-amber-700 bg-amber-100/80 px-2.5 py-1 rounded-lg inline-block border border-amber-200">
                          {ideas[0].votes_count}표
                        </span>
                      </div>
                      {/* 단상 기둥 */}
                      <div className="w-full h-28 bg-gradient-to-t from-amber-200 to-amber-100 rounded-t-xl border-t-2 border-amber-300 flex items-center justify-center font-black text-amber-500 text-3xl shadow-inner">
                        1
                      </div>
                    </motion.div>
                  )}
                  ...
                  {ideas[2] && (
                    ...
                    >
                      <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm w-full text-center mb-2 relative">
                        <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-amber-700/80 text-white text-[10px] font-black px-2 py-0.5 rounded-full shadow-sm">
                          3위
                        </span>
                        <h4 className="font-bold text-xs text-slate-800 mt-1 line-clamp-1">{ideas[2].title}</h4>
                        <p className="text-[10px] text-slate-400 mb-1">By {ideas[2].author}</p>
                        <span className="text-xs font-black text-blue-600">{ideas[2].votes_count}표</span>
                      </div>
                      {/* 단상 기둥 */}
                      <div className="w-full h-14 bg-gradient-to-t from-orange-100 to-amber-50 rounded-t-xl border-t-2 border-orange-200 flex items-center justify-center font-black text-orange-300 text-lg shadow-inner">
                        3
                      </div>
                    </motion.div>
                  )}
              ...
              {/* 2. 4위 이하 전체 리스트 (List View) */}
              {ideas.length > 3 && (
                <div className="space-y-2.5">
                  <h3 className="text-xs font-bold text-slate-400 mb-3 px-1">전체 순위</h3>
                  {ideas.slice(3).map((idea, index) => {
                    const rank = index + 4;
                    const percentage = (idea.votes_count / maxVotes) * 100;
                    ...
                    return (
                      <motion.div
                        ...
                        className="bg-white p-3.5 rounded-xl border border-slate-200/80 shadow-sm flex items-center gap-3.5 hover:border-slate-300 transition-colors"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-center mb-1.5">
                            <h4 className="text-xs font-bold text-slate-800 truncate">{idea.title}</h4>
