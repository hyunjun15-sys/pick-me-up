...
function OrbitingCharacter({
  ...
  const rx = 240; // 가로 궤도 크기
  const ry = 30;  // 세로 궤도 크기 (언덕 곡면에 딱 맞추도록 축소)

  const baseAngle = (index / Math.max(total, 1)) * 360;
  const currentAngleDeg = (baseAngle + time * 20) % 360;
  ...
  const bodyRotateY = cosVal > 0 ? -15 : 15; 

  return (
    <div
      className="absolute top-[78%] left-1/2 flex flex-col items-center pointer-events-auto transition-transform duration-75"
      style={{
    ...
    >
      ...
      <motion.div
        ...
        transition={{
          y: { repeat: Infinity, duration: 1.2 + index * 0.2, ease: 'easeInOut' },
      ...
      {/* 순위 블록 번호 표시 */}
      <div className="w-5 h-5 -mt-1 bg-white border border-slate-200 rounded-md shadow-sm flex items-center justify-center font-black text-[11px] text-slate-700">
        {rank}
      </div>

      {/* 이름표 */}
      <div className="mt-0.5 bg-white/90 backdrop-blur-sm px-2 py-0.5 rounded-lg shadow-sm border border-emerald-100 text-center max-w-[90px]">
        ...
        <div>
          ...
          {activeTab === 'leaderboard' && isAdminLoggedIn && (
            <div className="w-full rounded-3xl shadow-2xl relative overflow-hidden border border-emerald-200/80 bg-gradient-to-b from-sky-300 via-sky-100 to-emerald-100 p-6">
              ...
              {/* 초록색 잔디 언덕 바닥 */}
              <div className="absolute bottom-0 left-0 right-0 h-48 bg-gradient-to-t from-emerald-500 via-emerald-400 to-emerald-300 rounded-t-[50%] scale-x-125 border-t-4 border-emerald-300/50 shadow-inner" />
              ...
              {/* 무대 영역 */}
              <div
                className="relative z-20 h-[480px] w-full flex items-center justify-center overflow-hidden"
                  ...
                  {ideas.length > 0 && (
                    <>
                      ...
                      {leaderIdea && (() => {
                        ...
                        return (
                          <div
                            className="absolute top-[70%] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center"
                            style={{ zIndex: 25 }}
                          >
                            <motion.div
                              animate={{ y: [0, -3, 0] }}
                              transition={{ repeat: Infinity, duration: 1.5 }}
                              className="mb-1 px-3 py-0.5 bg-amber-400/95 text-slate-900 border-2 border-amber-200 rounded-full text-[11px] font-black shadow-lg flex items-center gap-1 whitespace-nowrap backdrop-blur-sm"
                            >
                              <span>👑 1위</span>
                            ...
                            <motion.div
                              ...
                              className="w-20 h-20 flex items-center justify-center filter drop-shadow-2xl"
                            >
                              <LeaderSVG className="w-full h-full" />
                            </motion.div>

                            {/* 1등 순위 블록 번호 표시 */}
                            <div className="w-6 h-6 -mt-1 bg-white border-2 border-amber-400 rounded-md shadow-md flex items-center justify-center font-black text-xs text-amber-600">
                              1
                            </div>
